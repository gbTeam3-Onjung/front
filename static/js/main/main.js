...
>
    <div>
        <a
            ...
            href=""
            target="_self"
            class="banner-container banner-box banner-link pc"
            ...
            <div class="banner-content-wrapper inner1">
                <div class="banner-content-wrapper inner2">
                    <div class="banner-content-wrapper inner3">
                        <span class="banner-color banner-web-sub-title">후원 게시판</span>
                        <span class="banner-color banner-web-title">아픈 딸에게 아무 것도 해줄 수가 없습니다</span>
                    </div>
                    <button class="banner-button-color2 banner-button-primary">
                        자세히 알아보기
                        <svg viewBox="0 0 12 12" class="arrow-inner banner-button-arrow">
                ...
                <img class="banner-web-image" src="../../static/images/banner2.jpg"/>
            </div>
        </a>
    </div>
...
>
    <div>
        <a
            ...
            href=""
            target="_blank"
            class="banner-container8 banner-box banner-link pc"
            style="width: 100%; display: inline-block;"
            id="last"
        >
            <div class="banner-content-wrapper inner1">
                <div class="banner-content-wrapper inner2">
                    <div class="banner-content-wrapper inner3">
                        <span class="banner-color banner-web-sub-title">후원게시판</span>
                        <span class="banner-color banner-web-title">학교에 가고 싶은 키르기스스탄 장애아동들을 도와주세요!</span>
                    </div>
                    <button class="banner-button-color2 banner-button-primary">
                        자세히 알아보기
                        <svg viewBox="0 0 12 12" class="arrow-inner banner-button-arrow">
                ...
                <img class="banner-web-image" src="../../static/images/banner9.jpg"/>
            </div>
        </a>
    </div>
</div>`; // 마지막 배너의 HTML 설정
...
arrows.forEach((arrow) => {
    arrow.addEventListener("click", (e) => {
        ...
        arrowCheck = false; // 중복 클릭을 방지하기 위해 플래그 설정
        ...
        if (arrowType === "left") {
            ...
            if (count == 0) {
                ...
                setTimeout(() => {
                    ...
                    banner.style.transform = `translate3d(-5310px, 0, 0)`; // 마지막 배너로 이동
                }, 500);
            ...
            if (count == 9) {
                ...
                setTimeout(() => {
                    banner.style.transition = `transform 0s`;
                    banner.style.transform = `translate3d(4248px, 0, 0)`; // 첫 번째 배너로 이동
                }, 500);
                count = 0; // 카운터를 첫 번째 배너로 초기화
            }
        }
...
document.addEventListener("DOMContentLoaded", function () {
    let roller = document.querySelector(".initial-child-container");
    roller.id = "roller1"; // 첫 번째 롤러 아이디 설정

    let clone = roller.cloneNode(true); // 자식까지 복제하도록 true 설정
    clone.id = "roller2"; // 복제본에 아이디 부여
    document.querySelector(".marquee").appendChild(clone); // marquee에 복제본 추가

    document.querySelector("#roller1").style.left = "0px"; // 첫 번째 배너 위치 설정
    document.querySelector("#roller2").style.left = roller.offsetWidth + "px"; // 두 번째 배너를 첫 번째 배너 크기만큼 옆에 위치시킴
