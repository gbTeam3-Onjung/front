...
const firstBanner = document.createElement("first"); // 첫 번째 배너를 위한 div 생성
const lastBanner = document.createElement("last"); // 마지막 배너를 위한 div 생성
...
banner.style.transform = `translate3d(-1,945.5px,0px,0px)`; // 초기 배너 위치 설정

console.log(firstBanner);

// 첫 번째 배너의 위치 (인덱스)를 확인하려면:
const firstBannerIndex = firstBanner.getAttribute("data-index");
console.log(`첫 번째 배너의 인덱스: ${firstBannerIndex}`);

// 자동 슬라이드를 실행하는 함수
const autoSlide = () => {
    count++; // 배너 위치를 한 칸 앞으로 이동
    banner.style.transition = `transform 0.5s`; // 배너 이동 애니메이션 설정
    banner.style.transform = `translate(-${1062 * count}px)`; // 배너 이동

    if (count == 10) {
        // 마지막 배너 위치를 넘었을 경우
        setTimeout(() => {
            banner.style.transition = `transform 0s`; // 애니메이션 없이 배너 위치 초기화
            banner.style.transform = `translate3d(-9379.5px,0px,0px)`;
        }, 500);
        ...
        if (arrowType === "left") {
            ...
            if (count == 0) {
                ...
                setTimeout(() => {
                    ...
                    banner.style.transform = `translate3d(-17875.5px,0px,0px)`; // 마지막 배너로 이동
                }, 500);
            ...
            count++; // 배너 위치를 한 칸 앞으로 이동
            banner.style.transition = `transform 0.5s`;
            banner.style.transform = `translate(-${1062 * count}px,0,0)`;

            if (count == 10) {
                // 마지막 배너 위치를 넘었을 경우
                setTimeout(() => {
                    banner.style.transition = `transform 0s`;
                    banner.style.transform = `translate3d(-1062px,0,0)`; // 첫 번째 배너로 이동
                }, 500);
